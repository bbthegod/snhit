...
export class PlayComponent implements OnInit {
  ...
  selectedProblem; // Vấn đề được lựa chọn

  selectIndex = 0; // Vị trí câu hỏi được lựa chọn
  selectProblemIndex = 0; // Vị trí câu hỏi được lựa chọn
  ...
  LastProblem() {
    if (this.selectProblemIndex != 0) {
      this.selectedProblem = this.listP[--this.selectProblemIndex];
    }
  }
  //Câu hỏi tiếp theo
  NextProblem() {
    if (this.selectProblemIndex != this.listP.length - 1) {
      this.selectedProblem = this.listP[++this.selectProblemIndex];
    }
  }
  setButton() {
